 ...
 * for the cloud-first QAgent workflow.
 */
...
export async function createPatchPR(
  ...
  const safePatchDesc = patch.description
    ...
    .replace(/[^a-z0-9]+/g, '-')
    .slice(0, 30);
  const branchName = `qagent/fix-${safePatchDesc}-${timestamp}`;
  ...
  // 7. Create PR
  const prTitle = `[QAgent] Fix: ${patch.description}`;
...
*This PR was automatically generated by [QAgent](https://github.com/qagent) - a self-healing QA agent.*
`;
